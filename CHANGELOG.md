...
- LZW 解码器改为直写输出：新增串长表 `strLen`，展开前即可确定串长，字符按位置直接写入输出缓冲，
  省掉原先「压入 pixelStack + 反向拷贝」的第二遍（原实现每像素写两次）。
  2400×1800 实测解码 LZW 12.875ms → 12.025ms（1.07x），解码结果字节一致。
  同一轮尝试且经实测否决的改动已写入 `LzwDecoder` 注释：一次补 4 字节的位缓冲（-12%）、
