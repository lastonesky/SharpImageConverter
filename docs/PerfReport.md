...
**提问**：编码的量化过程、解码的 LZW 过程能否通过 SIMD 提升性能？阈值：提升 <10% 视为无效。

**结论：两者都拿不到 ≥10%；量化侧在数学上被封死，解码侧的可向量化面只有约 1 ms。**
...
- quantize：59.5 → 54.8 ms，最多快 **8.0%**（未达 10%）
...
两者在指令级都不可向量化。**解码 LZW 的 SIMD 面 ≤1 ms < 10%，无效。**
